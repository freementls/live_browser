...
$sleeep = new sleeep();
$sleeep->full();
$sleeep->dump_total_time_taken();

class sleeep {

function __construct() {
	$this->sleeep_initial_time = sleeep::getmicrotime();
}
...
function cache_file_type_detection() {
	...
	foreach($this->files as $file) {
		...
		$file_type_from_contents = sleeep::file_type_from_contents($contents);
		$file_type_by_extension = substr(sleeep::file_extension($file), 1);
		...
		if($file_type_from_contents == NULL) { // programming languages can output any file extension
			...
			sleeep::fatal_error('file type missed!');
			...
			print('$contents: ');var_dump($contents);
			sleeep::fatal_error('file type mismatch!');
		} else {
			$matches++;
		}
	}
...
function file_type_from_contents($contents) {
	...
	if($file_type === false) {
		...
		sleeep::fatal_error('did not determine file type in digest()');
	}
	return $file_type;
}
...
function file_extension($string) {
	...
	$last_dot_position = sleeep::strpos_last($string, '.');
	$last_slash_position = sleeep::strpos_last($string, '/');
	$last_question_position = sleeep::strpos_last($string, '?');
	if($last_question_position === false) {
		$last_question_position = sleeep::strpos_last($string, '9o9quest9o9');
	}
	if($last_dot_position === false || $last_dot_position < $last_slash_position/* || $last_dot_position < sleeep::strpos_last($string, '\\')*/) {
		//return false;
	...
	if($last_question_position === false) {
		...
		$last_dot_between_slash_and_question_position = sleeep::strpos_last(substr($string, 0, $last_question_position), '.', $last_slash_position);
		if($last_question_position < $last_dot_between_slash_and_question_position) {
			...
			sleeep::fatal_error('not sure how to calculate file_extension from these');
			return false;
		} else {
...
function recursive_file_list_to_array($directory) {
	if(is_dir($directory)) {
		...
		while(FALSE !== ($entry = $d->read())) {
			...
			if(is_dir($Entry)) {
				$this->folder_counter++;
				sleeep::recursive_file_list_to_array($Entry);
				continue;
			} else {
				//print($Entry . "\r\n<br>");
				$this->files[] = $Entry;
...
function dump_total_time_taken() {
	$time_spent = sleeep::getmicrotime() - $this->sleeep_initial_time;
